...
Java_com_indiainsure_android_MB360_magisk_isMagiskPresentNative( JNIEnv *env,
                                                                   jobject this) {
    bool bRet = false;
    do {
...
JNIEXPORT jboolean JNICALL
Java_com_indiainsure_android_MB360_magisk_Native_isMagiskPresentNative(JNIEnv *env, jclass clazz) {
    bool bRet = false;
    do {
        bRet = is_supath_detected();
        if (bRet)
