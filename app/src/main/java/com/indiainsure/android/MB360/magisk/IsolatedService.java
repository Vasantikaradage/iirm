...
public class IsolatedService extends Service{
    ...
    private final IIsolatedService.Stub mBinder = new IIsolatedService.Stub(){
        public boolean isMagiskPresent(){
            ...
            mBinder.clearCallingIdentity();

            LogMyBenefits.d(TAG, "Isolated UID:"+ Os.getuid());
            ...
            try {
                ...
                while((str = reader.readLine()) != null && (count==0)){
                    ...
                    for(String path:blackListedMountPaths){
                        if(str.contains(path)){
                            LogMyBenefits.d(TAG, "Blacklisted Path found "+ path);
                            count++;
                            break;
                        }
                    }
                ...
                reader.close();
                fis.close();
                LogMyBenefits.d(TAG, "Count of detected paths "+ count);
                if(count > 0){
                    LogMyBenefits.d(TAG, "Found magisk in atleast 1 mount path ");
                    ...
                    LogMyBenefits.d(TAG, "Found Magisk in Native " + isMagiskPresent);
                }
